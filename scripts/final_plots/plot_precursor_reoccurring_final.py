...
for identifier_dict in list_of_things_to_plot:
	...
	plot_dict = {
		axes[0,0] : dict(field_name = [fld for fld in fields.keys() if 'Potential' in fld and 'VWS' in fld][0], letter='a'),
		axes[1,0] : dict(field_name = [fld for fld in fields.keys() if 'Robust' in fld and 'VWS' in fld][0], letter='c'),
		axes[0,1] : dict(field_name = [fld for fld in fields.keys() if 'Potential' in fld and 'SST' in fld][0], letter='b'),
		axes[1,1] : dict(field_name = [fld for fld in fields.keys() if 'Robust' in fld and 'SST' in fld][0], letter='d'),
	}

...
identifier_dict = {'$VWS_{MDR}$':'causal_ERA5_vws_lag2_X3_SST_n2','$SST_{MDR}$':'causal_ERA5_sst_lag2_X3_SST_n1'}
identifier_dict = {'$VWS_{MDR}$':'causal_JRA55_vws_lag2_X3_SST_n2','$SST_{MDR}$':'causal_JRA55_sst_lag2_X3_SST_n1'}

identifier_dict = {'$VWS_{MDR}$':'causal_ERA5_vws_lag4_X3_MSLP_wideClusters_n2','$SST_{MDR}$':'causal_ERA5_sst_lag4_X3_SST_dropSmall_n1'}
#identifier_dict = {'$VWS_{MDR}$':'causal_JRA55_vws_lag4_X3_MSLP_wideClusters_n2','$SST_{MDR}$':'causal_JRA55_sst_lag4_X3_SST_dropSmall_n1'}

identifier_dict = {'$VWS_{MDR}$':'causal_ERA5_vws_lag4_X3_SST_wideClusters_n2'}
...
plt.close('all')
fig,axes = plt.subplots(nrows=3, ncols=1, figsize=(4,4*120./180.+1), subplot_kw={'projection': ccrs.Robinson(central_longitude=central_longitude)}, gridspec_kw = {'height_ratios':[4,4,1]})

plot_dict = {
	axes[0] : dict(field_name = [fld for fld in fields.keys() if 'Potential' in fld and 'VWS' in fld][0], letter='a'),
	axes[1] : dict(field_name = [fld for fld in fields.keys() if 'Robust' in fld and 'VWS' in fld][0], letter='c'),
}
...
for ax,details in plot_dict.items():
	...
	#ax.set_title(field_name+' Precursors', fontsize=8)
...
# plt.tight_layout()
plt.savefig(tmp_path+'precursor/'+dataset+'/'+'_'.join([dataset,'lag'+str(lag),'causal'])+'_SST.png', dpi=300)

...
# Robustness and sets
#=====================================================================================
split_sets = pd.DataFrame(index=[np.array([2018]*len(all_years)).flatten(), np.array(list(all_years)).flatten()], columns=['train','test'])
for iy,year in enumerate([2018]):
	split_sets.loc[year,'train'] = True
	for i in range(3):
		split_sets.loc[year].loc[year-i, 'train'] = False

	split_sets.loc[year,'test'] = False
	split_sets.loc[year].loc[year, 'test'] = True
